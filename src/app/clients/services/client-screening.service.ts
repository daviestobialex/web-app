...
export class ClientScreeningService {
  ...
  /**
   * Treat a configured Yente URL as the source of truth for availability.
   *
   * Some deployments may omit the optional feature flag and only provide the
   * endpoint URL. In that case the UI should still render and allow manual
   * screening, otherwise the panel stays hidden and no request is ever sent.
   */
  get enabled(): boolean {
    return environment.enableYenteScreening === true || !!environment.yenteScreeningUrl?.trim();
  }
  ...
  private mapMatch(result: any): ClientScreeningMatch | null {
    ...
    return {
      ...
      sourceUrl:
        result?.sourceUrl ?? result?.first_seen ?? result?.referents?.[0] ?? result?.properties?.sourceUrl?.[0] ?? ''
    };
  }
