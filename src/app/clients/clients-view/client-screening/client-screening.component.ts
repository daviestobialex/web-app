...
import { ClientAddressRecord, ClientScreeningResult, ScreenableClient } from '../../models/client-screening.model';
...
export class ClientScreeningComponent implements OnChanges {
  ...
  shouldShowInfoMessage(result: ClientScreeningResult): boolean {
    return (
      !!result.screenedAt &&
      !result.errorMessageKey &&
      (result.status === 'clear' || result.status === 'unavailable') &&
      result.matches.length === 0
    );
  }

  infoMessageKey(result: ClientScreeningResult): string {
    if (result.status === 'unavailable' && result.type === 'address') {
      return 'labels.messages.No address data was found for this client';
    }

    return 'labels.messages.No screening matches were found';
  }
